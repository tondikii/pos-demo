...
|---|---|---|---|
| **Web Dashboard** | **SolidJS + SolidStart 1.x** (meta-framework) + **TailwindCSS** + **TanStack Query (solid-query)** | Solid 1.9+, SolidStart dengan SSR + file-based routing | SolidJS sesuai preferensi — performa tercepat (no VDOM), bundle kecil cocok untuk target UMKM dengan device/laptop low-end. SolidStart dipilih atas vanilla Solid karena butuh SSR untuk initial load cepat & proteksi route server-side. Alternatif (Vite SPA) ditolak: SEO dashboard tidak kritis tapi SSR bantu FCP & auth guard. |
| **Mobile Kasir (Android only)** | **Expo SDK 52+ + Expo Router + NativeWind** | React Native 0.76, Expo Router file-based | Expo sesuai preferensi. Expo Router = routing deklaratif seperti Next. NativeWind = Tailwind di RN. Eject tidak perlu di MVP. |
| **Backend API** | **ElysiaJS on Bun 1.2+** | Elysia 1.2, Bun runtime | Sesuai preferensi. Elysia = fastest Bun framework, type-safe (Eden), built-in OpenAPI. Cocok untuk volume tinggi margin tipis. Alternatif Hono/Nest ditolak: Elysia paling idiomatik di Bun. |
| **Landing Page (SEO)** | **Astro 5 + TailwindCSS** (terpisah dari dashboard) | Astro static + islands, `apps/landing` | **Keputusan best practice:** Pisah dari dashboard. Astro = 0 JS by default, image optimization, sitemap/OG/schema built-in, Lighthouse 95+ mudah. SolidStart bisa SSR tapi bundle dashboard (auth, query) ikut kebawa ke landing → lambat & SEO tidak optimal. Pisah = deploy edge murah, caching agresif, tidak ganggu deploy dashboard. Domain: `larispos.id` (landing), `app.larispos.id` (dashboard). |
| **Database** | **PostgreSQL 16 + Drizzle ORM** | Drizzle 0.38+, `drizzle-kit` | Sesuai preferensi. Relasional kuat untuk laporan agregasi, window function jam ramai. Drizzle = lightweight, SQL-like, type-safe, migrasi simpel — lebih ringan dari Prisma untuk infra lean. |
...
| **Monorepo Tooling** | **Bun workspaces + Turborepo** | `bun@1.2`, `turbo` | Bun = package manager + runtime (cepat, sesuai Elysia). Turborepo untuk task pipeline build/lint/test. |
